...
#+BEGIN_SRC emacs-lisp
  (defun lia--bind-some-keys-with-leader ()
    ...
    (lia-leader-def "n"     'lia/narrow-or-widen)
...
#+BEGIN_SRC emacs-lisp
  (use-package org
    ...
    (lia-leader-def "od" 'lia/goto-org-directory)
...
If there's multiple cursors, remove the extra cursors. Otherwise,
...
#+BEGIN_SRC emacs-lisp
  (defun lia/clear-mc-or-nohl ()
    "Remove multiple cursors or clear search highlight."
    (interactive)
    (cond ((and (featurep 'evil-mc) (evil-mc-has-cursors-p))
